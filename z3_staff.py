...
def clean_str(*args, **kwargs):
    '''Remove all characters that match regexes. utils'''
    num_strs = len(args)
...
def create_vars(num=0, start=0, step=1, type='BitVecs', size=64, prefix='x', g=globals()):
    ...
    exe_str = '%s = %s( "%s", %d )' % (
        t0, type, t1, size) if is_bitvecs else '%s = %s( "%s" )' % (t0, type, t1)
    if gDE_BUG:
...
def add_eq(*args, **kwargs):
    ...
    opt = kwargs['opt'] if 'opt' in kwargs else 3
    for i in args:
        for j in range(opt):
            i = simplify(i)
...
def set_ranges(rstart=32, rend=126, num=0, start=0, exclude=[], step=1, prefix='x'):
    '''Set ranges. By default set ASCII range for all variables. '''
    num = num if num != 0 else num_vars
    assert num > 0, 'num must be > 0'
    for i in range(start, num):  # start + num):
        need_continue = False
...
def prepare_founded_values(num=0, start=0, step=1, prefix='x', g=globals()):
    ...
    exe_str = '%s = %s' % (t0, t1)
    if gDE_BUG:
        print 'prepare_founded_values2:', clean_str(
            exe_str, rules=['g\["', '"\]', '\.as_long\(\)'])
    exec exe_str
    ret = [eval(var) for var in clean_str(t0, rules=[',']).split(' ')]
    if len(ret) == 1:
        return ret[0]
...
def all_is_not_eq(nums=[], prefix='x'):
    '''set all posible combinations of arr1 to != - not equal,
       e.g. for [1, 2, 3] -> s.add(p1 != 2, p1 != p3, p2 != 3)'''
    for subset in itertools.combinations(nums, 2):
        var_0 = '%s%d' % (prefix, subset[0])
        var_1 = '%s%d' % (prefix, subset[1])
